...
public class StopWords {
	...
	public String removeStopWords(String textFile) throws Exception {
		...
		Set stopWordsSet = StopFilter.makeStopSet(Version.LUCENE_36, stopWords,
				true);
	...
	public void calculateFrequency(String[] docs, RAMDirectory idx)
			throws IOException {
		IndexReader ir = IndexReader.open(idx);

		for (int docNum = 0; docNum < ir.numDocs(); docNum++) {
			TermFreqVector termFreqVector = ir.getTermFreqVector(docNum,
			...
			for (Map.Entry<String, Integer> entry : termFrequencyMap.entrySet()) {
				...
				count = (count == null) ? entry.getValue() : count
						+ entry.getValue();
	...
	private ArrayList<String> buildStopwordsList(int topCount) {
		...
		Map<String, Integer> sortByValue = SortingComparator
				.sortByValueDescending(frequencyCountMap);
		int count = 0;
